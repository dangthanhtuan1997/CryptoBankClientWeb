...
import { onGetOTPPassword, onResetPassword } from '../actions';
...
class ForgotPasswordPage extends React.Component {
  ...
  isValidForm = () => {
    const { username, password, rePassword, otp } = this.state;
    if (!username || !password || !rePassword || !otp) {
      return false;
    }
            ...
            <div className="nk-block nk-block-middle nk-auth-body">
              ...
              <div className="nk-block-head">
                <div className="nk-block-head-content">
                  <h5 className="nk-block-title">Quên mật khẩu</h5>
                </div>
              </div>{/* .nk-block-head */}
              <form>
                <div className="form-group">
                  ...
                  {this.state.username ? <input value={this.state.username} onChange={e => this.onChangeText('username', e.target.value)} type="text" className="form-control form-control-lg" id="username" placeholder="Nhập tên tài khoản của bạn" />
                    :
                    <input value={this.state.username} onChange={e => this.onChangeText('username', e.target.value)} type="text" className="form-control form-control-lg error" id="username" placeholder="Nhập tên tài khoản của bạn" />
                  }
                </div>{/* .foem-group */}
                <div className="form-group">
                  <div className="form-label-group">
                  ...
                  <div className="form-control-wrap">
                    ...
                    {this.state.password ?
                      <input value={this.state.password} onChange={e => this.onChangeText('password', e.target.value)} type="password" className="form-control form-control-lg" id="password" placeholder="Nhập mật khẩu mới của bạn" />
                      :
                      <input value={this.state.password} onChange={e => this.onChangeText('password', e.target.value)} type="password" className="form-control form-control-lg error" id="password" placeholder="Nhập mật khẩu mới của bạn" />
                    }
                  </div>
                </div>
                <div className="form-group">
                  <div className="form-label-group">
                    <label className="form-label" htmlFor="re-password">Xác nhận mật khẩu mới</label>
                  </div>
                  <div className="form-control-wrap">
                    <a tabIndex={-1} href="#" className="form-icon form-icon-right passcode-switch" data-target="password">
                      <em className="passcode-icon icon-show icon ni ni-eye" />
                ...
                <div className="form-group">
                  <div className="form-group">
                    <div className="form-label-group">
                      <label className="form-label" htmlFor="otp">Mã xác nhận</label>
                    </div>
                  </div>
                </div>
                <div className="row form-group">
                  <div className="col-6">
                    <div className="form-group">
                      <div className="form-control-wrap">
                        {this.state.otp ? <input value={this.state.otp} onChange={e => this.onChangeText('otp', e.target.value)} type="text" className="form-control form-control-lg" id="otp" placeholder="Mã xác nhận từ email đăng ký" />
                          :
                          <input value={this.state.otp} onChange={e => this.onChangeText('otp', e.target.value)} type="text" className="form-control form-control-lg error" id="otp" placeholder="Mã xác nhận từ email đăng ký" />
                        }
                      </div>
                    </div>
                  </div>
                  <div className="col-6">
                    <div className="form-group">
                      <button onClick={(event) => {
                        event.preventDefault();
                        if (this.isValidFormToGetOTP()) {
                          onGetOTPPassword(this.state.username);
                        }
                        ...
                        <span>Gửi mã xác nhận</span></button>
                    </div>
                  </div>
                </div>
              ...
              <div className="form-note-s2 pt-4">Trở về <a href="/login">Đăng nhập</a>
              </div>
...
export default connect(state => {
  ...
  return {
    onResetPassword: (username, password, otp) => dispatch(onResetPassword(username, password, otp))
  }
})(ForgotPasswordPage);